...
<notifications :duration="-1" group="foobar" position="top right" >
      ...
      <v-card color="red">
        <v-card-text>
          <div>
            <span class="font-italic white--text">fds is </span>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn dark small @click="emit(props.item.data.img, props.item.data.behavior, props.item.data.name)">send to teacher again</v-btn>
          <v-btn small @click="props.close" dark icon><v-icon>close</v-icon></v-btn>
        </v-card-actions>
      </v-card>
      </template>
    </notifications>
  <v-btn @click="foo">foo</v-btn>
  <v-btn @click="foobar">foobar</v-btn>
</div>
</template>
<script>
  export default ({
